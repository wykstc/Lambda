...
df_ids = pd.read_csv('supplement.csv',sep='\t', header=None, names=['ID','Text'])

df_txts = pd.read_csv('train.en.csv', sep='\t',header=None, names=['Text'])


with open('scoreAverage/deeper_dict.pkl', 'rb') as handle:
...
prompt_template = "Here is a ground truth sentence: {} And background sentences: {}; \n\
                Please judge the relation between them, ignore unrelated background sentences firstly; \n\
                Then use information from background sentences to enrich the ground truth sentence logically; \n\
                Only output enriched ground truth sentence directly without any other analysis."
def main(args):
    i = 0
    for idx, row in df_ids.iterrows():
        ...
        exit()
        txt = row['Text']
        background_sentences = text_dict.get(id, [])

        prompt_input = prompt_template.format(txt, background_sentences)
        ...
        with open('enrichedSentenceSupplement.txt', 'a', encoding='utf-8') as f:
